  ...
  return (
    <div className="min-h-screen bg-base-200 p-4 flex items-center justify-center">
      <div className="w-full max-w-2xl h-[calc(100vh-8rem)] bg-base-100 rounded-box shadow-lg flex flex-col">
        ...
        {/* 연결 상태 표시기 */}
        {!isConnected && (
          <div className="text-center p-2 bg-warning/10 text-warning text-sm">
            ⚠️ 채팅 서버에 연결 중입니다...
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          {messages.map((message) => (
            ...
            >
              <div className="chat-bubble whitespace-pre-wrap text-sm bg-opacity-90 flex items-center">
                {message.text}
              </div>
            </div>
          ))}
        ...
        <form onSubmit={handleSendMessage} className="p-4 border-t border-base-300">
          <div className="flex gap-3">
            <input
              ...
              value={inputMessage}
              disabled={!isConnected || !isInitialized || isLoading} // 로딩 중에는 입력 비활성화 가능
